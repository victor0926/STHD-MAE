...
class DiseaseClassifier(nn.Module):
    def __init__(self, input_dim=256, num_labels=5):
        super(DiseaseClassifier, self).__init__()
        # 全连接层，将输入的特征映射到一个较小的空间
        self.fc1 = nn.Linear(input_dim, 512)
        self.relu = nn.ReLU()  # 激活函数
        self.fc2 = nn.Linear(512, input_dim)  # 输出维度与输入相同
        self.softmax = nn.Softmax(dim=1)  # Softmax进行归一化
        # 最终的分类器：直接用一层全连接做分类
        self.fc3 = nn.Linear(input_dim, num_labels)  # 输出层，5个标签
    def forward(self, x):
        attention = self.fc1(x)  # (batch_size, patches, hidden_dim)
        attention = self.relu(attention)
        attention = self.fc2(attention)  # (batch_size, patches, features)
        # Softmax归一化得到权重（注意：softmax是针对patch维度进行的）
        attention_weights = self.softmax(attention)  # (batch_size, patches, features)

        # 取出 CLS 那行
        cls_weights = attention_weights[:, 0, :]  # shape (B, D)
        # ① 按特征平均，再按 batch 平均，得到一个标量
        mean_cls_weight = cls_weights.mean()  # ∈ [0,1]

        # 对每个patch加权平均，得到 (batch_size, features)
        weighted_x = x * attention_weights  # 通过权重进行加权
        x_weighted_avg = weighted_x.sum(dim=1)  # 在patch维度上求和
        # 直接通过全连接层做分类
        logits = self.fc3(x_weighted_avg)
        return logits
...
class MaskedAutoencoderViT(nn.Module):
    ...
    # def __init__(self, signal_length=1000, patch_size=50, in_chans=12,
        ...
        # self.classification = DiseaseClassifier(input_dim=256, num_labels=5)
        self.classification = DiseaseClassifier(input_dim=256, num_labels=44)
        # self.classification = DiseaseClassifier(input_dim=256, num_labels=19)
        # self.classification = DiseaseClassifier(input_dim=256, num_labels=4)
    ...
    def TimeView_masking(self, x, mask_ratio):
        ...
        x_masked = x_masked.contiguous().view(bs, 10*round(12 * (1 - mask_ratio)), 256)
    ...
    def forward_encoder(self, x, mask_ratio, epoch):
        ...
        for blk in self.blocks:
            x, share_att = blk(x)
            share_att_list.append(share_att)
            # patch2patch = att[:, :, 1:, 1:]
            # patch2cls = att[:, :, 1:, 0]
            # aaa = patch2cls + patch2patch.sum(dim=-1)
            # incoming_cls_ratio = patch2cls.mean()
            # outgoing_cls2patch = att[:, :, 0, 1:]
            # outgoing_ratio = outgoing_cls2patch.mean()
            # bbb = 1

        # # atts = atts[:,:,0:1,:,:]
        # avg_att = atts.mean(dim=(0, 1, 2))  # (P, P)
        # # Exclude global token as query (index 0)
        # eff_avg_att = avg_att[1:, :]  # (P-1, P)
        # # 假设你已经算出了 eff_avg_att 是一个 (P-1, P) 的 CUDA Tensor
        # eff_avg_att_np = eff_avg_att.detach().cpu().numpy()
        # # Plot with matplotlib
        # import matplotlib.pyplot as plt
        #
        # fig, ax = plt.subplots(figsize=(8, 6))
        # cax = ax.imshow(eff_avg_att_np, aspect='auto', origin='lower')
        #
        # # 标出 global token 列
        # # ax.axvline(x=0, color='white', linestyle='--', linewidth=2)
        #
        # # 设置 x 轴刻度：第一个为 'G'，其余为 1~P-1
        # patches = eff_avg_att_np.shape[1]
        # xticks = list(range(patches))
        # xtick_labels = ['G'] + [str(i) for i in range(1, patches)]
        # ax.set_xticks(xticks)
        # ax.set_xticklabels(xtick_labels, rotation=90)
        #
        # # 设置 y 轴刻度：0 对应 query patch 1，…，P-2 对应 query patch P-1
        # num_queries = eff_avg_att_np.shape[0]  # P-1
        # yticks = list(range(num_queries))
        # ytick_labels = [str(i) for i in range(1, num_queries + 1)]
        # ax.set_yticks(yticks)
        # ax.set_yticklabels(ytick_labels)
        #
        # # 标签和标题
        # ax.set_xlabel('Key Patch Index (G = global token)')
        # ax.set_ylabel('Query Patch Index')
        # ax.set_title('Average Attention Heatmap')
        #
        # # 加 colorbar
        ...
        # l_atts = torch.stack(l_att_list)  # (layers, B, H, P, P)
        # l_atts = l_atts.permute(1, 0, 2, 3, 4)
        # t_atts = torch.stack(t_att_list)  # (layers, B, H, P, P)
        # t_atts = t_atts.permute(1, 0, 2, 3, 4)
        share_atts = torch.stack(share_att_list)  # (layers, B, H, P, P)
        share_atts = share_atts.permute(1, 0, 2, 3, 4)
        layermean_atts = share_atts.mean(dim=1)
        headmearn_atts = share_atts.mean(dim=2)

        # return x
        return x, layermean_atts, headmearn_atts
    ...
    def forward(self, imgs, epoch, mask_ratio):
        bs, _, _ = imgs.shape
        latent, layermean_atts, headmearn_atts= self.forward_encoder(imgs, mask_ratio, epoch)
        ...
        logits = self.classification(latent)
        # return loss, pred_sig, pred_t, mask_t
        return logits, layermean_atts, headmearn_atts
